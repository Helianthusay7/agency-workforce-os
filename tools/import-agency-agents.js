...
function extractSectionItems(body, headingPattern, limit = 6) {
  ...
  for (const line of lines.slice(start + 1)) {
    ...
    if (value) items.push(value.slice(0, 140));
...
async function listMarkdownFiles(dir) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      if (!nonDivisionDirs.has(entry.name)) files.push(...await listMarkdownFiles(fullPath));
      continue;
    }
...
async function main() {
  ...
  const templates = (await Promise.all(files.map(buildTemplate))).filter(Boolean);
